...
const CATEGORIES: Apparel["category"][] = ["Tops", "Bottoms", "Outerwear", "Shoes", "Accessories"];
...
export function AddItemPage({ onSubmit, onCancel }: AddItemPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // map UI values to backend enums if needed
      // If your backend expects TOP/BOTTOM/... tell me; for now we send strings as-is
      const payload = {
        title: name,
  ...
  if (isSuccess) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-600 mb-8">Your item is now available for swapping on ReWear.</p>
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-gray-500 mt-1">Add your apparel to the swap marketplace</p>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="w-full space-y-1">
              ...
              <select
                id="category"
                value={category}
                onChange={(e) => setCategory(e.target.value as Apparel["category"])}
            ...
            {/* Size */}
            <div className="w-full space-y-1">
              <label htmlFor="size" className="text-sm font-medium text-neutral-700">
                Size
              </label>
              <select
                id="size"
            ...
            <div className="space-y-1">
              ...
              {previews.length > 0 ? (
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {previews.map((src, i) => (
                    <div key={i} className="rounded-lg overflow-hidden border border-neutral-200 bg-gray-50">
